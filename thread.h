...
private:
    ...
    int Destroy(void) {
        return pthread_mutex_destroy(&muLock);
    }

public:

    int Lock(void) {
        return pthread_mutex_lock(&muLock);
    } // gain access to the object (can block)

    int Unlock(void) {
        return pthread_mutex_unlock(&muLock);
    } // release the access to the object

    int TryLock(void) {
        int Err = pthread_mutex_lock(&muLock); // non-blocking attempt to lock
        return Err == EBUSY ? 1 : Err;
    } // 0 => lock succesfull, 1 => already locked by someone
};
...
private:
    ...
    int Destroy(void) {
        int Err = pthread_mutex_destroy(&muLock);
        if (Err != 0) return Err;
        Err = pthread_cond_destroy(&sigCond);
        if (Err != 0) return Err;
...
public:
    ...
    int Signal(void) {
        return pthread_cond_signal(&sigCond);
    } // signal to at least one thread, that something has changed

    int Broadcast(void) {
        return pthread_cond_broadcast(&sigCond);
    } // signal to all threads, that something has changed

    int Wait(void) {
        return pthread_cond_wait(&sigCond, &muLock);
    } // wait for a signal (can block)
...
public:
    ...
    int Init(void) {
        return pthread_rwlock_init(&rwLock, 0);
    }
...
public:
    ...
    Thread(void *(*Function)(void *) = 0) {
        ID = 0;
        setExec(Function);
    }

    void setExec(void *(*Function)(void *)) {
        Exec = Function;
    } // set the function to run by this thread
    ...
    int Join(void *&ExitStatus) // wait for the thread to terminate
    {
        ...
        ID = 0;
        return Ret;
    } // not clear how to behave if an error occures.
    ...
    {
        int Old;
        return pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, &Old);
    }

    static int CancelDisable(void) // I CAN NOT be cancelled
    {
        int Old;
        return pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, &Old);
    }
