...
public: // size must a power of 2: 2,4,8,16,32,64,128,256,...
    ...
    int Preset(int MaxSize) {
        ...
        BitRevIdx = new (std::nothrow) int [Size];
        if (BitRevIdx == 0) {
            Free();
            return 0;
        }
        Twiddle = new (std::nothrow) std::complex<Float> [Size];
        if (Twiddle == 0) {
            Free();
            return 0;
        }
    ...
    int Process(std::complex<Type> Data[], int Len) {
        ...
        int ShrinkShift = FindShrinkShift(Len);
        if (ShrinkShift < 0) return -1;
        Scramble(Data, ShrinkShift);
        CoreProc(Data, ShrinkShift);
        return 0;
    }
