...
public:
    ...
    int Preset(int Size, int Sign) {
        ...
        Free();
        Buffer = (std::complex<Float> *)fftw_malloc(Size * sizeof (std::complex<Float>));
    ...
    int PresetForward(int Size) {
        return Preset(Size, FFTW_FORWARD);
    }

    int PresetBackward(int Size) {
        return Preset(Size, FFTW_BACKWARD);
    }
...
public:
    ...
    int Preset(int Size, int Sign) {
        if ((Size == this->Size) && (Sign == this->Sign)) return Size;
        Free();
        Buffer = (std::complex<float> *)fftwf_malloc(Size * sizeof (std::complex<float>));
        if (Buffer == 0) return -1;
...
public:
    ...
    int Preset(int Size) { // if(Size==WindowSize) return Size;
        Free(); // deallocate everything
        if (FwdFFT.PresetForward(Size) < 0) return -1; // setup forward FFT
        WindowSize = Size;
        Window = new (std::nothrow) Float [WindowSize];
        if (Window == 0) return -1;
        Pipe = new (std::nothrow) std::complex<Float> [WindowSize];
        if (Pipe == 0) return -1;
        SetSineWindow();
        return Size;
    } // return FFT size (or negative when allocations failed)

    void Clear(void) {
        for (int Idx = 0; Idx < WindowSize; Idx++) {
            Pipe[Idx] = 0;
        }
        Ptr = WindowSize - SlideSize;
    }
...
public:
    ...
    int Preset(int Size) { // if(Size==WindowSize) return Size;                   // to avoid reallocations
        Free(); // deallocate everything
    ...
    int GetOutput(std::complex<OutType> *Output, int Decimate = 1) // Decimate must be a 1,2,4,8,16,...
    {
        ...
        for (Idx = 0; Idx < SlideSize; Idx += Decimate) {
            (*Output++) = Pipe[OutPtr];
            OutPtr += Decimate;
            if (OutPtr >= WindowSize) OutPtr -= WindowSize;
        }
...
public:
    ...
    int Preset(int Size, int Sign, int Jobs = 32) {
        ...
        if (Err < 0) {
            ...
            Size = 0;
            return Err;
        } // -1 => firmware up todate ?, -2 => Size not supported ?, -3 => not enough GPU memory
        this->Size = Size;
        this->Sign = Sign;
        this->Jobs = Jobs;
        return Size;
    ...
    int PresetForward(int Size, int Jobs = 32) {
        return Preset(Size, GPU_FFT_FWD, Jobs);
    }

    int PresetBackward(int Size, int Jobs = 32) {
        return Preset(Size, GPU_FFT_REV, Jobs);
    }

    std::complex<float> *Input(int Job = 0) {
        return (std::complex<float> *)(FFT->in + Job * FFT->step);
    }
    ...
    std::complex<float> *Output(int Job = 0) {
        return (std::complex<float> *)(FFT->out + Job * FFT->step);
    }
