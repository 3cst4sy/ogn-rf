...
int main(int argc, char **argv) {
    ...
    FFT.SetSineWindow(Window, FFTsize, (FloatType) (1.0 / sqrt(FFTsize)));
    ...
    if (RxGain >= 0) {
        ...
        SDR.setTunerGain(RxGain);
        printf("Tuner gain set to %3.1f dB (device reports %3.1f dB)\n", 0.1 * RxGain, 0.1 * SDR.getTunerGain());
    } else {
        SDR.setTunerGainAuto();
        printf("Tuner gain set to automatic\n");
    } // automatic gain control

    SDR.setOffsetTuning(RxOffsetTuning);
    if (RxOffsetTuning) printf("Offset tuning activated\n");
    ...
    for (int Scan = 0; Scan < Scans; Scan++, Freq += FreqStep) {
        ...
        if (Samples <= 0) {
            printf("SDR.Read(%5.1fMHz) failed\n", 1e-6 * Freq);
            continue;
        }
        ...
        char FileName[64];
        sprintf(FileName, "gsm_scan_%7.3fMHz-%7.3fMHz.jpg", 1e-6 * (Power.Freq - FreqStep / 2), 1e-6 * (Power.Freq + FreqStep / 2));
        MonoImage<FloatType> Spectrogram;
        Spectrogram.setExternal(Power.Data, FFTsize, Power.Samples());
        ...
        {
            FloatType CenterFreq = Chan*GSM_ChannelWidth;
