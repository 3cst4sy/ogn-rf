...
public:
    ...
    int set(const char *HostColonPort) {
        ...
        int Port;
        if (sscanf(Colon + 1, "%d", &Port) != 1) return -1;
        if ((Port < 0) || (Port > 0xFFFF)) return -1;
        setPort((unsigned short) Port);
        int Len = Colon - HostColonPort;
        if (Len >= TmpStringLen) return -1;
        memcpy(TmpString, HostColonPort, Len);
        TmpString[Len] = '\0';
    ...
    int setIP(const char *AsciiHost) {
        ...
        struct hostent *Host = gethostbyname(AsciiHost);
        if (Host == 0) return -1;
        char *AddrPtr = Host->h_addr_list[0];
        if (AddrPtr == 0) return -1;
    ...
    char *getHostName(void) {
        return getHostName(TmpString, TmpStringLen);
    }
...
public:
    ...
    size_t Relocate(size_t Size) {
        ...
        size_t Units = (Size + AllocUnit - 1) / AllocUnit;
        Size = Units*AllocUnit;
    ...
    int Delete(size_t Ofs, size_t DelLen) // delete some part of the data (involves memory move)
    {
        ...
        memcpy(Data + Ofs, Data + Ofs + DelLen, Len - DelLen);
        Len -= DelLen;
        Data[Len] = 0;
        return DelLen;
    }
    ...
    int SearchLineTerm(int StartIdx = 0) // search for a line terminator: \r or \n
    {
        ...
        return Idx - StartIdx;
    } // return the line length (not including the terminator)
...
public:
    ...
    int Create_STREAM(void) {
        return Create(SOCK_STREAM, IPPROTO_TCP);
    }

    int Create_DGRAM(void) {
        return Create(SOCK_DGRAM, 0);
    }
    ...
    int Connect(SocketAddress &ServerAddress) {
        if (SocketFile < 0) {
            if (Create_STREAM() < 0) return -1;
        } // if no socket yet, create a STREAM-type one.
