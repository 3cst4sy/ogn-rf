...
public:
    ...
    MonoImage(int NewWidth, int NewHeight) {
    ...
    int Reallocate(int NewSize) { // printf("PixelImage::Reallocate(%d) ...\n", NewSize);
        if (Allocated && (NewSize == Size)) return Size;
        Data = (Type *) realloc(Data, NewSize * sizeof (Type));
        if (Data == 0) {
            Allocated = 0;
            Size = 0;
            return 0;
    ...
    int New(int NewWidth, int NewHeight) { // printf("MonoImage::New(%dx%d) ...\n", NewWidth, NewHeight);
    ...
    void setPixel(int X, int Y, Type Value) {
    ...
    Type incPixel(int X, int Y, Type Value = 1) {
    ...
    int Add(ExtType *ExtData, int ExtWidth, int ExtHeight, ExtType Weight = 1) {
        if (Size == 0) {
            if (NewBlank(ExtWidth, ExtHeight) <= 0) return 0;
        }
    ...
    int CopyBox(ExtType *ExtData, int ExtWidth, int ExtHeight, int FirstCol, int FirstRow, int Cols, int Rows, ExtType Scale = 1) {
        ...
        for (int Row = 0; Row < Height; Row++) {
            ...
            if ((ExtRow < 0) || (ExtRow >= ExtHeight)) {
                for (int Col = 0; Col < Width; Col++) (*ImgData++) = 0;
                continue;
            }
