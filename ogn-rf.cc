...
public:
    ...
    void Config_Defaults(void) {
        SampleRate = 1000000;
        CenterFreq = 868300000;
        StartTime = 0.375;
        SamplesPerRead = (850 * SampleRate) / 1000;
        Gain = 600;
        DeviceIndex = 0;
        DeviceSerial[0] = 0;
        OffsetTuning = 0;
        FreqCorr = 0;
        GSM_CenterFreq = GSM_LowEdge + GSM_ScanStep / 2;
        GSM_Scan = 1;
        GSM_SamplesPerRead = (250 * SampleRate) / 1000;
        GSM_Gain = 200;
    }

    int config_lookup_float_or_int(config_t *Config, const char *Path, double *Value) {
        int Ret = config_lookup_float(Config, Path, Value);
        if (Ret == CONFIG_TRUE) return Ret;
        int IntValue;
    ...
    int Config(config_t *Config) {
        ...
        if (Serial) {
            strncpy(DeviceSerial, Serial, 12);
            DeviceSerial[11] = 0;
        }
        ...
        double InpGain = 60.0;
        config_lookup_float_or_int(Config, "RF.OGN.Gain", &InpGain);
        Gain = (int) floor(InpGain * 10 + 0.5);
        double Freq = 868.3;
        config_lookup_float_or_int(Config, "RF.OGN.CenterFreq", &Freq);
        CenterFreq = (int) floor(Freq * 1e6 + 0.5);
        InpGain = 20.0;
        config_lookup_float_or_int(Config, "RF.GSM.Gain", &InpGain);
        GSM_Gain = (int) floor(InpGain * 10 + 0.5);
        Freq = 958.4;
        config_lookup_float_or_int(Config, "RF.GSM.CenterFreq", &Freq);
        GSM_CenterFreq = (int) floor(Freq * 1e6 + 0.5);
        GSM_Scan = 0;
    ...
    void *Exec(void) { // printf("RF_Acq.Exec() ... Start\n");
        char Header[256];
        int Priority = Thr.getMaxPriority();
        ...
        while (!StopReq) {
            if (SDR.isOpen()) // if device is already open
            {
                ...
                int ReadGSM = (IntTimeNow % 10) == 0; // do the GSM calibration every 10 seconds
                double FracTimeNow = Now - IntTimeNow;
                double WaitTime = StartTime - FracTimeNow;
                ...
                if (WaitTime < 0.200) {
                    ...
                    if (Read > 0) // RF data Read() successful
                    {
                        ...
                        if (OutQueue.Size() < 4) {
                            ...
                            OutQueue.Recycle(Buffer);
                            printf("RF_Acq.Exec() ... Dropped a slot\n");
                        }
                    } else // RF data Read() failed
                    {
                        SDR.Close();
                        printf("RF_Acq.Exec() ... SDR.Read() failed => SDR.Close()\n");
                        continue;
                    }
                    if (ReadGSM) // if we are to read GSM in the second half-slot
                    {
                        ...
                        if (Read > 0) {
                            if (GSM_OutQueue.Size() < 3) GSM_OutQueue.Push(Buffer);
                                ...
                                GSM_OutQueue.Recycle(Buffer);
                                printf("RF_Acq.Exec() ... Dropped a GSM batch\n");
                            }
                        }
...
public:
    ...
    int WriteToPipe(void) // write OutBuffer to the output pipe
    {
        ...
        if (Len < 0) {
            printf("Inp_FFT.Exec() ... Error while writing to %s\n", OutPipeName);
            close(OutPipe);
            OutPipe = (-1);
            return -1;
        }
    ...
    static void *ThreadExec(void *Context) {
        Inp_FFT *This = (Inp_FFT *) Context;
        return This->Exec();
    }

    void *Exec(void) { // printf("Inp_FFT.Exec() ... Start\n");
        while (!StopReq) {
...
public:
    ...
    static void *ThreadExec(void *Context) {
        GSM_FFT *This = (GSM_FFT *) Context;
        return This->Exec();
    }

    void *Exec(void) { // printf("GSM_FFT.Exec() ... Start\n");
        while (!StopReq) {
            double ExecTime = getCPU();
    ...
    void Process(void) {
        ...
        {
            Float CenterFreq = Chan*ChanWidth;
            if (CenterFreq >= LastBinFreq) break; // center frequency of the channel
    ...
    static void AverPeakBkg(Float &Aver, Float &Peak, Float &PeakPos, Float &Bkg, Float *Data, int Size) {
        ...
        if (PeakIdx == 0) {
            Peak += Data[ 1];
            PeakPos = PeakIdx + Data[ 1] / Peak;
            Bkg = (Aver - Peak) / (Size - 2);
        } else if (PeakPos == (Size - 1)) {
            Peak += Data[Size - 2];
            PeakPos = PeakIdx - Data[Size - 2] / Peak;
            Bkg = (Aver - Peak) / (Size - 2);
        } else {
            Peak += Data[PeakIdx + 1] + Data[PeakIdx - 1];
            PeakPos = PeakIdx + (Data[PeakIdx + 1] - Data[PeakIdx - 1]) / Peak;
            Bkg = (Aver - Peak) / (Size - 3);
        }
        Aver /= Size;
    }
    ...
    int ProcessChan(Float &AverPower, int LowBin, int UppBin, Float CenterBin, Float BinWidth, Float CenterFreq) {
        ...
        int Bins = Power.Len;
        Aver.Allocate(1, Slides);
        Peak.Allocate(1, Slides);
        PeakPos.Allocate(1, Slides);
        Bkg.Allocate(1, Slides);
        ...
        Bkg.Full = Slides;
        Float PowerRMS;
        AverRMS(AverPower, PowerRMS, Aver.Data, Slides);
        // printf("AverPower=%3.1f, PowerRMS=%3.1f\n", AverPower, PowerRMS);
        if (PowerRMS > (0.5 * AverPower)) return 0; // skip pulsing channels

        Float AverPeak, PeakRMS;
        AverRMS(AverPeak, PeakRMS, Peak.Data, Slides);
        Float AverBkg, BkgRMS;
        AverRMS(AverBkg, BkgRMS, Bkg.Data, Slides);
...
public:
    ...
    HTTP_Server(RF_Acq *RF, GSM_FFT<Float> *GSM) {
        ...
        this->GSM = GSM;
        Host[0] = 0;
        SocketAddress::getHostName(Host, 32);
        Config_Defaults();
    }
    ...
    int Config(config_t *Config) {
        config_lookup_int(Config, "HTTP.Port", &Port);
        return 0;
    }
    ...
    static void *ThreadExec(void *Context) {
        HTTP_Server *This = (HTTP_Server *) Context;
        return This->Exec();
    }
    ...
    void *Exec(void) {
        ...
        while (1) {
            Socket Listen;
            if (Listen.Create_STREAM() < 0) {
                printf("HTTP_Server.Exec() ... Cannot Create_STREAM()\n");
                sleep(1);
                continue;
            }
            if (Listen.setReuseAddress() < 0) {
                printf("HTTP_Server.Exec() ... Cannot setReuseAddress()\n");
                sleep(1);
                continue;
            }
            if (Listen.Listen(Port) < 0) {
                printf("HTTP_Server.Exec() ... Cannot listen() on port %d\n", Port);
                sleep(1);
                continue;
            }
            ...
            while (1) {
                ...
                if (Listen.Accept(*Client, ClientAddress) < 0) {
                    printf("HTTP_Server.Exec() ... Cannot accept()\n");
                    sleep(1);
                    break;
                }
                printf("HTTP_Server.Exec() ... Client from %s\n", ClientAddress.getIPColonPort());
                Client->setReceiveTimeout(2.0);
                Client->setSendTimeout(5.0);
                Client->setLinger(1, 5);
                SocketBuffer Request;
                time_t ConnectTime;
                time(&ConnectTime);
                while (1) {
                    if (Client->Receive(Request) < 0) {
                        printf("HTTP_Server.Exec() ... Cannot receive()\n");
                        Client->SendShutdown();
                        Client->Close();
                        delete Client;
                        Client = 0;
                    ...
                    time_t Now;
                    time(&Now);
                    if ((Now - ConnectTime) > 2) {
                        printf("HTTP_Server.Exec() ... Request timeout\n");
                        Client->SendShutdown();
                        Client->Close();
                        delete Client;
                        Client = 0;
    ...
    void ProcessRequest(Socket *Client, SocketBuffer &Request) {
        ...
        char File[64];
        if (CopyWord(File, Request.Data + 4, 64) < 0) goto BadRequest;
